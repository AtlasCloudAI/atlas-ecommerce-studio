...
// 一键部署:把整套 AI Studio 克隆到用户自己的 Vercel / Cloudflare 账户。
const REPO = 'https://github.com/AtlasCloudAI/atlas-saas-starter';
const ENV_VARS = 'ATLASCLOUD_API_KEY,DATABASE_URL,NEXTAUTH_SECRET,GOOGLE_CLIENT_ID,GOOGLE_CLIENT_SECRET';
const ENV_DESC = 'Atlas API key, database URL, NextAuth secret, Google OAuth credentials (see .env.example)';
...
const VERCEL_URL =
  ...
  `&project-name=my-ai-studio&repository-name=my-ai-studio`;
      ...
      {open && (
        <div className="fixed inset-0 z-[60] bg-black/70 backdrop-blur grid place-items-center p-4" onClick={() => setOpen(false)}>
          <div className="relative w-full max-w-md rounded-2xl border border-white/10 bg-[#1c1e21] p-6 text-[#f7f7f8]" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-sm text-white/50 mb-5">{zh ? '一键把整套 AI Studio 克隆到你自己的账户,填入你的 Atlas key 就能上线运营。' : 'Clone the entire AI Studio to your own account — add your Atlas key and go live.'}</p>
            ...
            <p className="text-[11px] text-white/30 mt-4">{zh ? '部署后在平台填入环境变量(Atlas key / 数据库 / Google OAuth),详见 ' : 'After deploying, set env vars (Atlas key / database / Google OAuth) on the platform — see '}<a href={ENV_LINK} target="_blank" rel="noopener noreferrer" className="underline hover:text-white/60">.env.example</a></p>
          </div>
        </div>
      )}
    </>
